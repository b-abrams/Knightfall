...
public class EnemyDamageThrustingOnly : MonoBehaviour {
	...
	{
		if(target == thrust && collider.IsTouching(target))
		{
			OnExplode();
		}
	}
	...
	{
		if (target.collider == thrust && collider.IsTouching(target.collider))
		{
			OnExplode();
		}
	}
